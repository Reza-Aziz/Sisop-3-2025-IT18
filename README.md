...
2. Fungsi untuk mencatat aktivitas client.
<pre>
 void log_action(const char *msg) {
    printf("%s\n", msg);
}
...
3. Fungsi untuk membuat koneksi ke server.
<pre>
 int connect_server() {
    int sock = socket(AF_INET, SOCK_STREAM, 0);
...
4. Fungsi untuk mengirim file ke server.
<pre>
...
* Membuka file dari direktori SECRET_DIR
